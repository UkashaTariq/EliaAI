...
//     if (useWebsets) {
//       // Use Exa Websets approach
//       contacts = await handleWebsetsSearch(query, maxResults);
//     } else {
//       // Use regular Exa search (fallback)
//       contacts = await handleRegularSearch(query, maxResults);
//     }
...
//       search: {
//         query: query,
//         numResults: Math.min(maxResults, 50),
...
async function handleWebsetsSearch(
  ...
  try {
    ...
    const allItems: any[] = [];
    let offset = 0;
    const limit = Math.min(batchSize, 100); // Exa API might have per-request limits
    ...
    while (offset < maxResults) {
      ...
      offset += itemsResponse.data.length;

      console.log(`Fetched ${allItems.length} items so far...`);

      // If we got less than requested, we've reached the end
      if (itemsResponse.data.length < limit) {
        break;
      }
